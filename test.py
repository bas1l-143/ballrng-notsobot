...
if arg:
    if arg=="get":
        ...
        if len(args)>2:
            discord["storage"]["user"][args[1]] = args[2]
            print(discord["storage"]["user"][args[1]])
            exit()
        else:
            print("nothin?")
            exit()
...
if remain:
    print(f"Rolls per command are capped to 12h of afk, use this tag repeatedly to roll the remaining {remain*ROLLRATE} ")
